...
{ 
	...
	public: 
		...
		{ 
			...
			else cout<<"L'inserimento della chiave "<<key<<" avvenuto correttamente " <<endl;
			return depth >= 0; 
		} 
		...
		{
			if(searchKey(key) != NULL)  return 1;
			return 0;
		}
		...
		{
			if(node == NULL) return node;
			if(node->getRight()) return minimum(node->getRight());
		...
		Nodo<T> *deleteKey(T key) 
		{
			...
			if(node->getLeft()==NULL) {
				...
				if( node->getRight() ) node->getRight()->setParent(parent);
				return node;
			}
...
{ 
	...
	sgt->insertKey(8); 
	sgt->insertKey(8.5); 
	printf("Preorder traversal of the" " constructed ScapeGoat tree is \n"); 
	sgt->_preorder();
	cout<<endl; 
